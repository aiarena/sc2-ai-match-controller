...
pub fn latest_executable_path(versions_dir: PathBuf) -> io::Result<PathBuf> {
    let (max_version, path) = fs::read_dir(versions_dir)?
        ...
        .max_by_key(|(v, _)| *v)
        .ok_or_else(|| {
            std::io::Error::new(ErrorKind::NotFound, "No downloaded SC2 binaries found")
        })?;
