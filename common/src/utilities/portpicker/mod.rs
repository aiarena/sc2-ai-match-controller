...
/// ```rust
/// use common::portpicker::pick_unused_port;
...
pub fn pick_unused_port_in_range(mut range: std::ops::Range<Port>) -> Option<Port> {
    let between = Uniform::from(range.clone());
    let mut rng = rand::thread_rng();

    // Try random ports first
    for _ in 0..10 {
        let port = between.sample(&mut rng);
        if is_free(port) {
            return Some(port);
        }
    }
